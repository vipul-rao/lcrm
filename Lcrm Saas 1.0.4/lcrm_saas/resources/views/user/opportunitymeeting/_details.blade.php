                    ...
                    <div class="controls">
                        @if(isset($meeting))
                            @foreach(explode(',',$meeting->staff_attendees) as $staff_attendees)
                                {{ $opportunity->user->where('id',$staff_attendees)->first()->full_name }}
                                @if(!@$loop->last)
                                    ,
                                @endif
                            @endforeach
        ...
        <div class="form-group">
            <div class="controls">
                @if (@$action == trans('action.show'))
                    <a href="{{ url($type.'/'.$opportunity->id) }}" class="btn btn-primary"><i class="fa fa-arrow-left"></i> {{trans('table.close')}}</a>
                @else
