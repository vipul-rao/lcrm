            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group required {{ $errors->has('company_id') ? 'has-error' : '' }}">
                        {!! Form::label('company_id', trans('sales_order.company_id'), ['class' => 'control-label required']) !!}
                        <div class="controls">
                            {!! Form::select('company_id', isset($companies)?$companies:[''=>trans('sales_order.company_id')], null, ['class' => 'form-control']) !!}
                ...
                <div class="col-md-6">
                    <div class="form-group required {{ $errors->has('sales_team_id') ? 'has-error' : '' }}">
                        {!! Form::label('sales_team_id', trans('sales_order.sales_team_id'), ['class' => 'control-label  required']) !!}
                        <div class="controls">
                            {!! Form::select('sales_team_id', $salesteams, (isset($saleorder)?$saleorder->sales_team_id:null), ['class' => 'form-control']) !!}
            ...
            <div class="row">
                <div class="col-12">
                    <div class="form-group required {{ $errors->has('qtemplate_id') ? 'has-error' : '' }}">
                        {!! Form::label('qtemplate_id', trans('sales_order.quotation_template'), ['class' => 'control-label']) !!}
                        <div class="controls">
                            {!! Form::select('qtemplate_id', $qtemplates, (isset($saleorder)?$saleorder->qtemplate_id:null), ['class' => 'form-control']) !!}
                ...
                <div class="col-md-6">
                    <div class="form-group required {{ $errors->has('date') ? 'has-error' : '' }}">
                        {!! Form::label('date', trans('sales_order.date'), ['class' => 'control-label required']) !!}
                        <div class="controls">
                            {!! Form::text('date', isset($saleorder) ? $saleorder->start_date : null, ['class' => 'form-control']) !!}
                ...
                <div class="col-md-6">
                    <div class="form-group required {{ $errors->has('exp_date') ? 'has-error' : '' }}">
                        {!! Form::label('exp_date', trans('sales_order.exp_date'), ['class' => 'control-label required']) !!}
                        <div class="controls">
                            {!! Form::text('exp_date', isset($saleorder) ? $saleorder->expire_date : null, ['class' => 'form-control']) !!}
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group required {{ $errors->has('payment_term') ? 'has-error' : '' }}">
                        {!! Form::label('payment_term', trans('sales_order.payment_term'), ['class' => 'control-label required']) !!}
                        <div class="controls">
                            <select name="payment_term" class="form-control" id="payment_term">
                                <option value=""></option>
                                @if($payment_term1!='0' && $payment_term1!='')
                                    <option value="{{$payment_term1.' '.trans('sales_order.days')}}"
                                            @if(isset($saleorder) && $payment_term1.' '.trans('sales_order.days') == $saleorder->payment_term) selected @endif>{{$payment_term1}} {{trans('sales_order.days')}}</option>
                                @endif
                                @if($payment_term2!='0' && $payment_term2!='')
                                    <option value="{{$payment_term2.' '.trans('sales_order.days')}}"
                                            @if(isset($saleorder) && $payment_term2.' '.trans('sales_order.days') == $saleorder->payment_term) selected @endif>{{$payment_term2}} {{trans('sales_order.days')}}</option>
                                @endif
                                @if($payment_term3!='0' && $payment_term3!='')
                                    <option value="{{$payment_term3.' '.trans('sales_order.days')}}"
                                            @if(isset($saleorder) && $payment_term3.' '.trans('sales_order.days')== $saleorder->payment_term) selected @endif>{{$payment_term3}} {{trans('sales_order.days')}}</option>
                                @endif
                                <option value="0 {{trans('sales_order.days')}}"
                                        @if(isset($saleorder) && $saleorder->payment_term.' '.trans('sales_order.days')==0) selected @endif>{{trans('sales_order.immediate_payment')}}</option>
                            </select>
                ...
                <div class="col-sm-6">
                    <div class="form-group required {{ $errors->has('status') ? 'has-error' : '' }}">
                        {!! Form::label('status', trans('sales_order.status'), ['class' => 'control-label required']) !!}
                        <div class="controls">
                            <div class="input-group">
                                <label>
                                    <input type="radio" name="status" value="{{trans('sales_order.draft_salesorder')}}"
                                           class='icheckblue'
                                           @if(isset($saleorder) && old('status',$saleorder->status)==trans('sales_order.draft_salesorder') || old('status')==trans('sales_order.draft_salesorder')) checked @endif>
                                    {{trans('sales_order.draft_salesorder')}}
                                </label>
                                <label>
                                    <input type="radio" name="status" value="{{trans('sales_order.send_salesorder')}}"
                                           class='icheckblue'
                                           @if(isset($saleorder) && old('status',$saleorder->status)==trans('sales_order.send_salesorder') || old('status')==trans('sales_order.send_salesorder')) checked @endif>
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr style="font-size: 12px;">
                                <th>{{trans('sales_order.product')}}</th>
                                <th>{{trans('sales_order.description')}}</th>
                                <th>{{trans('sales_order.quantity')}}</th>
                                <th>{{trans('sales_order.unit_price')}}</th>
                                <th>{{trans('sales_order.subtotal')}}</th>
                                <th></th>
                            </tr>
                            </thead>
                    ...
                    <button type="button" id="AddMoreFile"
                            class="btn btn-sm btn-primary"><i class="fa fa-plus"></i> {{trans('sales_order.add_product')}}
                    </button>
                    <div class="row">&nbsp;</div>
                </div>
            </div>
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="form-group required {{ $errors->has('discount') ? 'has-error' : '' }}">
                        {!! Form::label('discount', trans('sales_order.discount').' (%)', ['class' => 'control-label']) !!}
                        <div class="controls">
                ...
                <div class="col-md-12">
                    <div class="form-group required {{ $errors->has('grand_total') ? 'has-error' : '' }}">
                        {!! Form::label('grand_total', trans('sales_order.grand_total'), ['class' => 'control-label']) !!}
                        <div class="controls">
                ...
                <div class="col-md-6">
                    <div class="form-group required {{ $errors->has('tax_amount') ? 'has-error' : '' }}">
                        {!! Form::label('tax_amount', trans('sales_order.tax_amount').' ('.$sales_tax.'%)', ['class' => 'control-label']) !!}
                        <div class="controls">
                ...
                <div class="col-md-12">
                    <div class="form-group required {{ $errors->has('final_price') ? 'has-error' : '' }}">
                        {!! Form::label('final_price', trans('sales_order.final_price'), ['class' => 'control-label']) !!}
                        <div class="controls">
...
@section('scripts')
    <script>
        $(document).ready(function(){
            $("#company_id").select2({
                theme:"bootstrap",
                placeholder:"{{ trans('sales_order.company_id') }}"
            });
            $("#sales_team_id").select2({
                theme:"bootstrap",
                placeholder:"{{ trans('sales_order.sales_team_id') }}"
            });
            $("#qtemplate_id").select2({
                theme:"bootstrap",
                placeholder:"{{ trans('sales_order.quotation_template') }}"
            });
            $("#recipients").select2({
                theme:"bootstrap",
        ...
        $(function () {
            ...
            $('#qtemplate_id').change(function () {
                if ($(this).val() > 0) {
                    $.ajax({
                        type: "GET",
                        url: '{{url("sales_order/ajax_qtemplates_products")}}/' + $(this).val(),
