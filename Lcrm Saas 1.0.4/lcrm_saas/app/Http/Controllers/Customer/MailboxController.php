...
class MailboxController extends Controller
{
        ...
        $users_list = $this->organizationRepository->getUserStaffCustomers()->get()
            ->map(function ($user) {
                return [
                    ...
                    'active' => (isset($user->last_login) && $user->last_login >= now()->subMinutes('15')->toDateTimeString()) ? '1' : '0',
                ];
            });
    ...
    public function deleteMail(Email $mail)
    {
    ...
    public function postReply($id, Request $request)
    {
        ...
        $subject = (substr($orgMail->subject, 0, strlen('Re:')) === 'Re:')
            ?
            ($orgMail->subject)
            :
            ('Re: ' . $orgMail->subject);
